...
| **Project Completion Video (PCV)** | **[youtu.be/_-VUvYbxI-Y](https://youtu.be/_-VUvYbxI-Y)** |
| Developer workshop recording (in-repo) | [docs/media/workshop.mp4](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/media/workshop.mp4) |
| Demo recording (in-repo) | [docs/media/demo.mp4](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/media/demo.mp4) |
...
**On-chain evidence — Hydra Head L1 lifecycle + L2 transactions from the recorded workshop run** (full details in [docs/onchain-evidence.md](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/onchain-evidence.md)): the `hydra-node` 1.3.0 instance powering the [workshop recording](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/media/workshop.mp4) **observed `OnInitTx` + `OnCommitTx` + `OnCollectComTx`** (head id `6f66666c696e652d0001`, decoded ASCII `offline-0001`, single-party head, 43,200 s contestation period, **1,000,000,000 lovelace ≈ 1000 tADA** committed for `addr_test1qq8ac7q…`); the SDK then submitted **17 Hydra L2 transactions** (`NewTx` round-trip via `HydraHttpClient.submitTx` and re-observed as `TxValid` over the WebSocket) — `08b4b80751c6ab6d30693af76839f28a3bb09394d875864c08275ef622a0a4a1`, `169cb4be096079cb150f13f48bc795ffc47fda3913110dc5e41a80fbb8c9641f`, …, full list in [onchain-evidence.md](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/onchain-evidence.md). The L1 close → contest → fanout paths (`OnCloseTx` / `OnContestTx` / `OnFanoutTx`) are exposed by the SDK via `HydraHeadFacade.sendClose / sendSafeClose / sendContest / sendFanout` and are asserted deterministically in [`open_close_flow_test.dart`](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/packages/hydra_client/test/open_close_flow_test.dart) (9 cases walking Idle → Initial → Open → Closed → FanoutPossible).
...
**Project Completion Video (PCV):** [youtu.be/_-VUvYbxI-Y](https://youtu.be/_-VUvYbxI-Y) — public YouTube link with no access restrictions, audio commentary in English. Supplementary in-repo recordings: developer onboarding workshop walkthrough at [docs/media/workshop.mp4](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/media/workshop.mp4) (9 min), in-app demo at [docs/media/demo.mp4](https://github.com/dextrlabs-dev/hydra-mobile-sdk/blob/main/docs/media/demo.mp4).
